...
fn main() {
    ...
    println!(
        "cargo::rustc-link-search=native={manifest_dir}/booster_robotics_sdk/lib/{architecture}"
    );
    println!(
        "cargo::rustc-link-search=native={manifest_dir}/booster_robotics_sdk/third_party/lib/{architecture}"
    );
